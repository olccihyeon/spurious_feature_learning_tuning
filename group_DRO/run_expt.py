...
def get_parser():
    ...
    # Settings
    parser.add_argument(
        "-d", "--dataset", choices=list(dataset_attributes.keys()) + ["CivilComments", "FMOW"], required=False
    )
    ...
    # Model
    parser.add_argument("--model", choices=list(model_attributes.keys()) + dir(dfr_models), default="resnet50")
...
def main(args):
    ...
    if args.dfr_model:
        ...
        model_cls = getattr(dfr_models, args.model)
