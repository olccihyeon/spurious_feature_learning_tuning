...
def get_args():
    ...
    parser.add_argument("--result_path", type=str, default="logs/", help="Path to save results")
    parser.add_argument("--ckpt_path", type=str, default=None, required=False, help="Checkpoint path")
    parser.add_argument("--batch_size", type=int, default=100, required=False, help="Checkpoint path")
    parser.add_argument("--save_embeddings", action="store_true", help="Save embeddings on disc")
    parser.add_argument(
        "--predict_spurious", action="store_true", help="Predict spurious attribute instead of class label"
    )
    parser.add_argument("--drop_group", type=int, default=None, required=False, help="Drop group from evaluation")
    parser.add_argument("--log_dir", type=str, default="", help="For loading wandb results")
    parser.add_argument("--save_linear_model", action="store_true", help="Save linear model weights")
    parser.add_argument("--save_best_epoch", action="store_true", help="Save best epoch num to pkl")
    # DFR TR
    parser.add_argument("--dfr_train", action="store_true", help="Use train data for reweighting")
    parser.add_argument("--DFR_data", default="base", type=str)
    parser.add_argument(
        "--normalize_before",
        action="store_true",
    )
...
def dfr_on_validation_tune(all_embeddings, all_y, all_g, preprocess=True, num_retrains=1):
    worst_accs = {}
    for i in range(num_retrains):
        ...
        for c in C_OPTIONS:
            ...
            group_accs = np.array([(preds_val == y_val)[g_val == g].mean() for g in range(n_groups)])
...
def dfr_on_validation_eval(
    ...
    if preprocess:
        scaler = StandardScaler()
        scaler.fit(dfr_embeddings["val"])
    for i in range(num_retrains):
        for _ in range(20):
            x_val = dfr_embeddings["val"]
            y_val = dfr_y["val"]
            g_val = dfr_g["val"]
            n_groups = np.max(g_val) + 1
            ...
            if np.any(np.unique(y_train) != np.unique(dfr_y["val"])):
    ...
    train_accs = [(preds_train == y_train)[g_train == g].mean() for g in range(n_groups)]

    if args.save_linear_model:
        linear_model = {"coef": logreg.coef_, "intercept": logreg.intercept_, "scaler": scaler}
        ...
        linear_model_path = os.path.join(
            dir_linear_model, os.path.basename(args.result_path)[:-4] + f"_linear_model_{target_type}.pkl"
        )
...
def main(args):
    if has_wandb:
        wandb.init(project="DFR_spurious_feature_training")
    ...
    train_loader, test_loader_dict, get_ys_func = utils.get_data(
        args, logger, contrastive=False, dataset=args.data_dir
    )
    fg_train_loader, fg_test_loader_dict, _ = utils.get_data(args, logger, contrastive=False, dataset=args.fgdata_dir)
    bg_train_loader, bg_test_loader_dict, _ = utils.get_data(args, logger, contrastive=False, dataset=args.bgdata_dir)
    ...
    base_model_results = {name: utils.get_results(accs, get_ys_func) for name, accs in base_model_results.items()}
    print(base_model_results)
    print()

    model.fc = torch.nn.Identity()
    # splits = ["test", "val"]
    splits = {"test": test_loader_dict["test"], "val": test_loader_dict["val"]}
    splits_fg = {"test": fg_test_loader_dict["test"], "val": fg_test_loader_dict["val"]}
    splits_bg = {"test": bg_test_loader_dict["test"], "val": bg_test_loader_dict["val"]}
    if args.dfr_train:
    ...
    if os.path.exists(f"{args.result_path[:-4]}.npz"):
        ...
        all_embeddings_bg = {}
        all_y_bg, all_p_bg, all_g_bg = {}, {}, {}
        for name, loader in splits_bg.items():
            all_embeddings_bg[name] = []
            all_y_bg[name], all_p_bg[name], all_g_bg[name] = [], [], []
            for x, y, g, p in tqdm.tqdm(loader):
                with torch.no_grad():
                    all_embeddings_bg[name].append(model(x.cuda()).detach().cpu().numpy())
                    all_y_bg[name].append(y.detach().cpu().numpy())
                    all_g_bg[name].append(g.detach().cpu().numpy())
                    all_p_bg[name].append(p.detach().cpu().numpy())

            all_embeddings_bg[name] = np.vstack(all_embeddings_bg[name])
            all_y_bg[name] = np.concatenate(all_y_bg[name])
            all_g_bg[name] = np.concatenate(all_g_bg[name])
            all_p_bg[name] = np.concatenate(all_p_bg[name])

        all_embeddings_fg = {}
        all_y_fg, all_p_fg, all_g_fg = {}, {}, {}
        for name, loader in splits_fg.items():
            all_embeddings_fg[name] = []
            all_y_fg[name], all_p_fg[name], all_g_fg[name] = [], [], []
            for x, y, g, p in tqdm.tqdm(loader):
                with torch.no_grad():
                    all_embeddings_fg[name].append(model(x.cuda()).detach().cpu().numpy())
                    all_y_fg[name].append(y.detach().cpu().numpy())
                    all_g_fg[name].append(g.detach().cpu().numpy())
                    all_p_fg[name].append(p.detach().cpu().numpy())
            all_embeddings_fg[name] = np.vstack(all_embeddings_fg[name])
            all_y_fg[name] = np.concatenate(all_y_fg[name])
            all_g_fg[name] = np.concatenate(all_g_fg[name])
            all_p_fg[name] = np.concatenate(all_p_fg[name])

        if args.save_embeddings:
            np.savez(
                f"{args.result_path[:-4]}.npz",
    ...
    print("DFR")
    dfr_results = {}
    if args.DFR_data == "base":
        c = dfr_on_validation_tune(all_embeddings, all_y, all_g, all_p)
    ...
    elif args.DFR_data == "direct_fg":
        c = dfr_on_validation_tune(all_embeddings_fg, all_y_fg, all_g_fg, all_p_fg)
        dfr_results["best_hypers"] = c
        print("Hypers:", (c))
        test_accs, test_mean_acc, train_accs = dfr_on_validation_eval(
            args,
            c,
            all_embeddings=all_embeddings,
            dfr_embeddings=all_embeddings_fg,
            all_y=all_y,
            dfr_y=all_y_fg,
            all_g=all_g,
            dfr_g=all_g_fg,
    ...
    elif args.DFR_data == "indirect_fg":
        if args.normalize_before:
            scaler = StandardScaler()
            scaler.fit(all_embeddings["val"])
            scaler.fit(all_embeddings_bg["val"])
        ...
        for name in all_embeddings:
            result_embeddings[name] = all_embeddings[name] - all_embeddings_bg[name]

        for key in all_y.keys():
            result_y[key] = all_y[key]
        for key in all_g.keys():
            result_g[key] = all_g[key]
        for key in all_p.keys():
            result_p[key] = all_p[key]

        c = dfr_on_validation_tune(result_embeddings, result_y, result_g, all_p_fg)
        dfr_results["best_hypers"] = c
        print("Hypers:", (c))
        test_accs, test_mean_acc, train_accs = dfr_on_validation_eval(
            args,
            c,
            all_embeddings=all_embeddings,
            dfr_embeddings=result_embeddings,
            all_y=all_y,
            dfr_y=result_y,
            all_g=all_g,
            dfr_g=result_g,
    ...
    if has_wandb:
        ...
        wandb.log({"DFR_data": args.DFR_data, "normalize_before": args.normalize_before})
