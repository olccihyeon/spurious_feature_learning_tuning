...
def get_results(acc_groups, get_ys_func):
    ...
    results = {f"accuracy_{get_ys_func(g)[0]}_{get_ys_func(g)[1]}": acc_groups[g].avg for g in groups}
...
def get_model_dataset_args():
    ...
    parser.add_argument(
        "--test_data_dir", type=str, default=None, required=False, help="Test data directory (default: <data_dir>)"
    )
    parser.add_argument(
        "--data_transform",
        type=str,
...
def get_default_args():
    ...
    parser.add_argument("--tuning", default="retraining", type=str, choices=["retraining", "finetuning"])
    ...
    parser.add_argument(
        ...
        choices=["cosine_lr_scheduler", "constant_lr_scheduler", "bert_lr_scheduler"],
        help="Scheduler name",
    )
...
def get_data(args, logger=None, contrastive=False, finetune_on_val=False, dataset=None):
    ...
    if finetune_on_val:
        ...
        trainset = dataset_cls(basedir=dataset, split="val", transform=train_transform)
        data.balance_groups(trainset)
    else:
        trainset = dataset_cls(basedir=dataset, split="train", transform=train_transform)

    data.remove_minority_groups(trainset, args.num_minority_groups_remove)

    test_data_dir = args.test_data_dir if args.test_data_dir else dataset
    testset_dict = {
        split: dataset_cls(basedir=test_data_dir, split=split, transform=test_transform) for split in ["test", "val"]
    }

    collate_fn = data.get_collate_fn(mixup=args.mixup, num_classes=trainset.n_classes)
    ...
    train_loader = DataLoader(trainset, shuffle=train_shuffle, sampler=sampler, collate_fn=collate_fn, **loader_kwargs)
    test_loader_dict = {name: DataLoader(ds, shuffle=False, **loader_kwargs) for name, ds in testset_dict.items()}

    get_ys_func = partial(get_y_s, n_spurious=testset_dict["test"].n_spurious)
    if logger is not None:
        logging_utils.log_data(logger, trainset, testset_dict["test"], testset_dict["val"], get_ys_func=get_ys_func)

    if contrastive:
        finetune_transform = transform_cls(train=True, finetune=True)
        trainset_finetune = dataset_cls(basedir=dataset, split="train", transform=finetune_transform)
        finetune_loader = DataLoader(
            trainset_finetune, shuffle=False if sampler else True, sampler=sampler, **loader_kwargs
        )
...
def get_data_base(args, logger=None, contrastive=False, finetune_on_val=False):
    if args.data_transform == "None":
        transform_cls = lambda *args, **kwargs: None
    else:
        transform_cls = getattr(data, args.data_transform)
    ...
    if finetune_on_val:
        # TODO: train or test transform
        trainset = dataset_cls(basedir=args.data_dir, split="val", transform=train_transform)
        data.balance_groups(trainset)
    else:
    ...
    if contrastive:
        finetune_transform = transform_cls(train=True, finetune=True)
        trainset_finetune = dataset_cls(basedir=args.data_dir, split="train", transform=finetune_transform)
        finetune_loader = DataLoader(
            trainset_finetune, shuffle=False if sampler else True, sampler=sampler, **loader_kwargs
        )
        return train_loader, finetune_loader, test_loader_dict, get_ys_func
