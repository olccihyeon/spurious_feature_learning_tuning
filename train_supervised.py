...
def main(args):
    ...
    writer = None
    # Data
    train_loader, test_loader_dict, get_ys_func = utils.get_data_base(args, logger, contrastive=False)
    ...
    optimizer = getattr(optimizers, args.optimizer)(
        model, lr=args.init_lr, momentum=args.momentum_decay, weight_decay=args.weight_decay
    )
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        #     logger, writer, epoch, loss_meter, acc_groups, get_ys_func, images=example_images, tag=train_tag
        # )
        ...
        if epoch % args.eval_freq == 0:
            ...
            #     utils.log_test_results(logger, writer, epoch, acc_groups, get_ys_func, ds_name)
            ...
            if best_val_wga is None or val_wga > best_val_wga:
                ...
                torch.save(model.state_dict(), os.path.join(args.output_dir, f"best_checkpoint_{args.seed}.pt"))

        if epoch % args.save_freq == 0:  # and epoch > 0:
            torch.save(model.state_dict(), os.path.join(args.output_dir, f"checkpoint_{epoch}.pt"))
        # logger.write("\n")

    torch.save(model.state_dict(), os.path.join(args.output_dir, f"final_checkpoint_{args.seed}.pt"))
